// =============================================================================
// SurApícola — Pantalla de Cuenta Corriente de Proveedores (Prompt 4)
// =============================================================================
...
import { useCuentaProveedor } from '../hooks/useCuentaCorriente';
...
type RouteProps = RouteProp<TabParamList, 'ProveedorCuenta'>;

export function ProveedorCuentaScreen() {
  const route = useRoute<RouteProps>();
  const navigation = useNavigation<BottomTabNavigationProp<TabParamList>>();
  const { proveedorId } = route.params;

  const { resumen, movimientos, loading, error, registrarPago } = useCuentaProveedor(proveedorId);
  ...
  const formatCategoria = (cat: string) => {
    if (cat === 'miel_panales') return '🍯 Miel y Panales';
    if (cat === 'envases') return '🧴 Envases e Insumos';
    return '📦 Otros';
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.resumenCard}>
          <View style={styles.nombreRow}>
            <Text style={styles.proveedorNombre}>{resumen?.nombre}</Text>
            <View style={styles.categoriaBadge}>
              <Text style={styles.categoriaBadgeText}>
                {resumen?.categoria ? formatCategoria(resumen.categoria) : ''}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.kpiRow}>
            <View style={styles.kpiCol}>
              ...
              <Text style={[styles.kpiValue, { color: COLORS.text }]}>
                {formatearDinero(resumen?.totalComprado ?? 0)}
              </Text>
            </View>
            <View style={styles.kpiCol}>
              <Text style={styles.kpiLabel}>Total Pagado</Text>
              <Text style={[styles.kpiValue, { color: COLORS.success }]}>
                {formatearDinero(resumen?.totalPagado ?? 0)}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.saldoRow}>
            <Text style={styles.saldoLabel}>Saldo de Deuda</Text>
          ...
          <View style={styles.fechasRow}>
            <Text style={styles.fechaText}>
              Última compra: <Text style={{ color: COLORS.text }}>{resumen?.ultimaCompraFecha ? formatearFecha(resumen.ultimaCompraFecha) : 'Ninguna'}</Text>
            </Text>
            <Text style={styles.fechaText}>
              Último pago: <Text style={{ color: COLORS.text }}>{resumen?.ultimoPagoFecha ? formatearFecha(resumen.ultimoPagoFecha) : 'Ninguno'}</Text>
            </Text>
          </View>
          ...
          {resumen && resumen.saldoPendiente > 0 ? (
            ...
            >
              <Text style={styles.btnPagarText}>💸 Registrar pago</Text>
            </TouchableOpacity>
          ) : (
        ...
        {movimientos.length === 0 ? (
          <View style={styles.vacioCard}>
            <Text style={styles.vacioTexto}>El proveedor no registra movimientos vigentes.</Text>
          </View>
        ) : (
          movimientos.map((mov) => {
            const esCompra = mov.tipo === 'compra';
            const sign = esCompra ? '+' : '-';
            const color = esCompra ? COLORS.text : COLORS.success;
            return (
              <View key={`${mov.tipo}-${mov.id}`} style={styles.movimientoCard}>
                <View style={styles.movLeft}>
                  <View style={[styles.tipoBadge, { backgroundColor: esCompra ? '#2A2A3E' : '#1B3A2B' }]}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.formGroup}>
              ...
              />
              <Text style={styles.helpText}>Deuda máxima: {formatearDinero(resumen?.saldoPendiente ?? 0)}</Text>
            </View>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  if (centavos > (resumen?.saldoPendiente ?? 0)) {
                    Alert.alert('Error', 'El pago no puede ser mayor al saldo de deuda.');
                    return;
                  }
                  ...
                  setGuardando(true);
                  const exito = await registrarPago(centavos, fecha, medioPago, notas.trim() || null);
                  setGuardando(false);
                  
                  if (exito) {
                    setModalVisible(false);
                    Alert.alert('Éxito', 'Pago registrado y saldo de deuda actualizado.');
                  } else {
